...
class Scatt_TwoOrder(nn.Module):
    def __init__(self, psi_num, C, S):
        ...
        self.pad = nn.ZeroPad2d(3)
        #self.eps = 1e-2
        #self.penelty = nn.Parameter(torch.randn(1))
...
# =============================================================================
        ...
        #u1Psi = torch.sqrt(self.psi_real(x)**2 + self.psi_imag(x)**2 + self.eps)
        #u1Psi = torch.sqrt(self.psi_real(x)**2 + self.psi_imag(x)**2) * (1-self.penelty)
        u1Psi = torch.sqrt(self.psi_real(x)**2 + self.psi_imag(x)**2)
        ...
        #s1Phi = F.conv2d(self.pad(s1), self.phi.weight.data.repeat(self.psi_num+1, 1, 1, 1), bias=None, stride=self.S[1], groups=s1.size(1)) * self.penelty
